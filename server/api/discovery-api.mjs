...
async function createStudioBrief(request, response, context) {
  ...
  const studioBusiness = {
    ...
    description: `En ${businessName} te atendemos en ${city} con servicios de ${category.toLowerCase()} y contacto directo para resolver lo que necesitas.`,
    conversionGoal: `${brief.suggestedCTA} y atender consultas de ${city}`,
    announcement: `Ya puedes contactar con ${businessName} para consultar horario, ubicacion y disponibilidad.`,
    phone,
    address: cleanText(source.address, 360),
    hours: openingHours,
    services: suggestedServices(categoryKey),
    features: [
      source.reviews ? `Valoracion de clientes con ${Number(source.reviews)} resenas` : "Negocio localizado en un directorio geografico",
      city ? `Estamos en ${city} y te indicamos como llegar` : "Te indicamos ubicacion y forma de contacto",
...
function suggestedServices(category) {
  if (includesAny(category, ["restaurant", "restaurante", "bar", "cafe"])) return [
  ...
  if (includesAny(category, ["clinic", "clinica", "dentist", "dentista"])) return [
    ...
    "Tratamientos: atencion por area con duracion y cuidados indicados antes de reservar.",
  ...
  if (includesAny(category, ["workshop", "taller"])) return [
    ...
    "Presupuesto: orientacion de precio y plazo antes de confirmar."
  ];
  return [
