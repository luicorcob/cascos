...
El módulo premium de recomendaciones locales usa Supabase/PostgreSQL, Overpass, Wikidata, Wikipedia, Wikimedia Commons y Leaflet. No realiza llamadas a modelos de lenguaje. Descubre POI tanto en OpenStreetMap como directamente en el índice geográfico de Wikipedia, cruza duplicados por nombre y distancia y prioriza búsquedas específicas de playas, patrimonio, edificios documentados, espacios culturales, parques, plazas y miradores. Las consultas de Overpass se dividen por familias y disponen de un endpoint alternativo para conservar cobertura cuando la instancia principal está saturada. Los extractos documentales se guardan una vez y las tarjetas muestran atribución visible de la fuente. Para POI de OSM sin foto, contrasta páginas e imágenes cercanas mediante nombre y coordenadas; las coincidencias débiles y los recursos de marca (logos, escudos o SVG) se descartan. La selección pública pondera calidad documental, imagen, interés turístico, diversidad y distancia, en ese orden de conjunto, en lugar de limitarse a los puntos más próximos.
